...
searchButton.addEventListener('click', function() {
    ...
    fetch("https://api.openweathermap.org/data/2.5/weather?q="+inputKeyword.value+"&appid=f4f7614f818e607db93499fadb85db18&units=metric")
        .then(response => response.json())
        .then(response => {
            let result = document.querySelector('.hasil')

            result.innerHTML = `<div class="card"><div class="card-body"><h2 style="margin-bottom: 15px;">${response.name}, ${response.sys.country}</h2>
                                ...
                                <h4 style="color: #012443;">Geo Coordinates : [${response.coord.lat}, ${response.coord.lon}]</h4></div></div>` 
        })
